...
if __name__ == "__main__":
    ...
    gen_specs = GenSpecs(
        ...
        user={
            "initial_sample_size": 0,  # H0 provides the sample points
            ...
            "max_active_runs": 6,
            "lb": np.array([-1.39, 2.33]),  # Updated lower bounds
            "ub": np.array([-0.1, 3.5])  # Updated upper bounds
        }
    )
    ...
    _H0_all = np.load("run_example_history_length=302_evals=300_workers=4.npy")
    _lb = gen_specs.user["lb"]
    _ub = gen_specs.user["ub"]
    _completed = _H0_all["sim_ended"]
    ...
    if len(H0) > 0:
        ...
        # Renumber sim_ids to be sequential (required by libEnsemble history)
        H0["sim_id"] = np.arange(len(H0))
        print("Using", len(H0), "of", len(_H0_all), "completed points within current bounds")
    else:
        H0 = None
