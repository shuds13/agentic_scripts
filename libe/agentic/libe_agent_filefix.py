...
def augment_error_with_file_info(error_msg):
    """If error mentions file issues, list files in mentioned directories"""
    error_lower = error_msg.lower()
    if not any(x in error_lower for x in ["notfound", "not found", "no such file", "cannot find"]):
        return error_msg
    
    augmented_info = []
    seen_dirs = set()
    
    # Find anything with slashes (likely a path)
    for word in error_msg.split():
        if '/' in word:
            try:
                ...
                if parent_dir.exists() and str(parent_dir) not in seen_dirs:
                    seen_dirs.add(str(parent_dir))
                    files = sorted([f.name for f in parent_dir.iterdir() if f.is_file()])[:20]
                    if files:
                        augmented_info.append(
                            f"\nFiles in '{parent_dir}': {', '.join(files)}"
                        )
            except (ValueError, OSError):
                pass
    
    return error_msg + "".join(augmented_info) if augmented_info else error_msg
...
async def fix_scripts(agent, scripts_text, error_msg, run_script_name):
    ...
    # Augment error message with file info if file-not-found errors detected
    augmented_error = augment_error_with_file_info(error_msg)
    
    fix_prompt = FIX_PROMPT_TEMPLATE.format(
        error_msg=augmented_error, 
